...
def load_split_data(train_path, test_path):
    ...
    def create_class_label(row):
        if row['Label'] == 'iO':
            ...
            error = str(row['Error_Description']).strip().lower()
            if 'oil' in error:
                return 'Oil'
            elif 'tilt' in error:
                return 'Tilt'
    ...
    train_df = train_df[train_df['Class'] != 'Unknown']
    test_df = test_df[test_df['Class'] != 'Unknown']
...
def forward_selection(X_train, y_train, model, cv=5, max_features=20, verbose=True, random_state=42):
    """
...
def main():
    ...
    if SAVE_PLOTS and history:
        plot_selection_history(history, "forward_selection_lr.png")
    ...
    if SAVE_PLOTS:
        plot_confusion_matrix(avg_cm, class_names, 
                              'LR Confusion Matrix (TEST)', 'confusion_matrix_lr.png')
        
        # Last trial with integer values
        plt.figure(figsize=(10, 8))
        sns.heatmap(last_cm, annot=True, fmt='d', cmap='Blues',
                    xticklabels=class_names, yticklabels=class_names,
                    annot_kws={'size': 14})
        plt.title('LR Confusion Matrix - Last Trial (TEST)', fontsize=16, fontweight='bold')
        plt.xlabel('Predicted Label')
        plt.ylabel('True Label')
        plt.tight_layout()
        plt.savefig('confusion_matrix_lr_last.png', dpi=150, bbox_inches='tight')
        print("Saved: confusion_matrix_lr_last.png")
        plt.show()
